...
void		Hud::fillElements(std::vector<char> unitsValue, unsigned int units)
{
	...
	for (unsigned int i = units; i >= 0 && iValue < unitsValue.size(); i--)
...
void		Hud::newElement(void)
{
	...
	element->setPosition(Vec3(1. - (element->getScale().getX() + 0.01)
...
void		Hud::display(int fps)
{
	...
	{
		Vec3 scale = this->hudElements[i]->getScale();
